...
- **FR-01: Location Input**
  ผู้ใช้สามารถกรอกพิกัด Latitude / Longitude หรือใช้ตำแหน่งปัจจุบันผ่าน GPS ได้
- **FR-02: Radius Search**
  ผู้ใช้สามารถกำหนดรัศมีการค้นหาสถานที่ใกล้เคียงได้ เช่น 3 km, 8 km, 15 km
- **FR-03: Category Filter**
  ผู้ใช้สามารถเลือกหมวดหมู่สถานที่ได้ เช่น ทั้งหมด, โรงแรม, ร้านอาหาร, กีฬา, ท่องเที่ยว
- **FR-04: Place Scanner Result**
  ระบบสามารถแสดงผลสถานที่ที่ค้นพบ พร้อมชื่อสถานที่ หมวดหมู่ ระยะทาง คะแนน และที่อยู่
- **FR-05: Map Display**
  ระบบสามารถแสดงตำแหน่งผู้ใช้และสถานที่ที่พบในแผนที่ โดยใช้ Leaflet.js
- **FR-06: Favorite Places**
  ผู้ใช้สามารถบันทึก/ยกเลิกสถานที่โปรดได้ และข้อมูลยังคงอยู่หลัง refresh หน้าเว็บ
- **FR-07: 1-Day Trip Planner**
  ผู้ใช้สามารถเพิ่มกิจกรรมหรือสถานที่ลงในแผนทริป 1 วัน พร้อมกำหนดเวลาเริ่มต้นและสิ้นสุด
- **FR-08: Time Conflict Detection**
  ระบบต้องตรวจสอบเวลาทับซ้อนใน Trip Planner และแจ้งเตือนเมื่อกิจกรรมชนกัน
- **FR-09: Language & Theme Toggle**
  ผู้ใช้สามารถเปลี่ยนภาษา TH/EN และเปลี่ยน Light/Dark Theme ได้ โดยค่าถูกบันทึกไว้ข้ามหน้า
- **FR-10: Multi-page Navigation**
  ผู้ใช้สามารถใช้งานได้ 3 หน้า ได้แก่ Scanner, Favorites และ Trip Planner

### Non-Functional Requirements (NFR)

- **NFR-01: Usability**
  ระบบต้องใช้งานง่าย ผู้ใช้สามารถค้นหาสถานที่และวางแผนทริปได้ด้วยขั้นตอนที่ไม่ซับซ้อน
- **NFR-02: Performance**
  ระบบควรแสดงผลการค้นหาและการเปลี่ยนหน้าภายในเวลาที่เหมาะสม โดยไม่ทำให้ UI ค้าง
- **NFR-03: Reliability**
  ระบบต้องตรวจสอบข้อมูลผิดพลาด เช่น พิกัดไม่ถูกต้อง เวลาเริ่มมากกว่าเวลาสิ้นสุด หรือเวลาทับซ้อน
- **NFR-04: Persistence**
  ข้อมูลสำคัญ เช่น Favorites, Trip Plan, Language และ Theme ต้องถูกเก็บไว้ใน localStorage เพื่อไม่หายหลัง refresh
- **NFR-05: Maintainability**
  โค้ดควรแยกเป็นโมดูล เช่น Models, Map module, Trip Planner module, Helper functions เพื่อให้ง่ายต่อการแก้ไขและต่อยอด
- **NFR-06: Testability**
  ระบบต้องมี Unit Test และ UI E2E Test เพื่อยืนยันการทำงานของ business logic และ user workflow หลัก
- **NFR-07: Security**
...
## สิ่งเปลี่ยนแปลงจากรายงาน Phase 1 และ 2 และเหตุผลที่เปลี่ยน
...
1. การปรับหน้าจอผู้ใช้งาน (UI/UX)
   
	- สิ่งที่เปลี่ยน: เพิ่มฟีเจอร์การสลับ Theme (Light/Dark Mode) และการเปลี่ยนภาษา (Thai/English) ซึ่งไม่มีใน Prototype เดิม
...
3. การเปลี่ยนรูปแบบการเชื่อมต่อข้อมูล (API Integration):

	- สิ่งที่เปลี่ยน: จากเดิมที่ออกแบบไว้ว่าอาจจะใช้ Mock Database สลับมาใช้การดึงข้อมูลสถานที่จริงผ่าน SerpAPI (Google Maps Engine)

 	- เหตุผล: เพื่อให้ระบบค้นหาสถานที่ (Scan) ได้ข้อมูลที่แม่นยำ อัปเดตแบบ Real-time และครอบคลุมหมวดหมู่สถานที่ได้ตรงตามที่ออกแบบไว้ (FR-02 และ FR-03)

4. การเพิ่มความเข้มงวดของ Logic ใน Trip Planner:
...
- **LCP & CLS:** จุดกึ่งกลางในการโหลดกระดานหลักรวดเร็วขึ้นมาก (ลดลงเกือบวินาที) และความนิ่งของ Layout (CLS) สมบูรณ์แบบที่ค่า 0 จึงทำให้คะแนนรวมดึงขึ้นได้สูง
...
![ci_pipeline](docs/ci_cd_pipeline/ci_pipeline_success.png)
