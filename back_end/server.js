...
// Load .env.local from back_end/ (same folder as server.js)
...
async function handleImageProxy(req, res) {
  try {
    ...
    if (!imageUrl || !/^https?:\/\//i.test(imageUrl)) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ error: 'Missing or invalid image url' }));
      return;
    }
    ...
    res.end(JSON.stringify({ error: 'Could not load image' }));
  }
}
